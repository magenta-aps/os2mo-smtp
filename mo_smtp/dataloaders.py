...
class DataLoader:
    ...
    async def get_org_unit_relations(mo: GraphQLClient, org_unit_uuid: list[UUID]):
        gql_response = await mo.get_org_unit_relations(org_unit_uuid)
        return gql_response.objects

    async def get_institution_address(mo: GraphQLClient, uuids: UUID, root: UUID):
        gql_response = await mo.get_institution_address(uuids, root)
        addresses = set(
            [address.value for address in one(gql_response.objects).current.addresses]
        )
