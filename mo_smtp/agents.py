...
from .autogenerated_graphql_client.related_unit_registrations import (
...
async def handle_related_units(
    ...
    all_registrations = await get_related_units_data(mo, related_units_uuid=uuid)
    registrations = [
        r
    ...
    if not len(registrations):
        logger.info("Related units not found")
        return

    # Collect all unique lønorg unit UUIDs across all registrations and
    # validity periods, since a deleted relation won't have a current state.
    loen_org_unit_uuids: set[UUID] = set()
