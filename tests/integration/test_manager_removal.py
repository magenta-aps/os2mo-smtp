...
async def _setup_org_and_employee(
    ...
    """Helper: create root org unit, child org unit, and employee. Returns (org_unit_uuid, employee_uuid)."""
...
async def test_vacant_manager_sends_email(
    ...
    email_client.send_email.assert_called_once()
    message = email_client.send_email.call_args.args[2]
...
async def test_terminated_manager_on_root_uses_root_email(
    ...
    email_client.send_email.assert_called_once()
    receiver = email_client.send_email.call_args.args[0]
...
async def test_manager_with_multiple_validities_current_is_picked(
    ...
    extract_current_or_latest_validity picks the current one (no email sent
    because the manager is still employed)."""
    org_unit_uuid, employee_uuid = await _setup_org_and_employee(
        graphql_client, root_loen_org
    )
