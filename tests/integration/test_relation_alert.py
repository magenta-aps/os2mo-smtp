...
@pytest.mark.integration_test
async def test_alert_already_sent_is_skipped(
    context,
    session: AsyncSession,
    graphql_client: GraphQLClient,
    email_client: MagicMock,
    ...
    """Handling the same org unit twice only produces one email — the second
    call finds a SentAlert and returns early."""
    org_unit_type = (await graphql_client._testing__get_org_unit_type()).objects[0].uuid
