"""Integration tests for the org unit relation alert flow.

These tests require the full OS2mo stack running (MO, Keycloak, AMQP, DB).
"""
...
async def test_sends_alert_to_receivers(
    ...
    """When use_org_unit_emails is false, alert is sent to configured receivers."""
    ...
    await handle_org_unit(context, child.uuid, None, graphql_client, session)

    email_client.send_email.assert_called_once()
    call_kwargs = email_client.send_email.call_args
    assert "datagruppen@silkeborg.dk" in str(call_kwargs)
...
async def test_sends_alert_to_org_unit_email(
    ...
    """When use_org_unit_emails is true, alert is sent to the org unit's email."""
    org_unit_type = (await graphql_client._testing__get_org_unit_type()).objects[0].uuid
    ...
    email_client.send_email.assert_called_once()
    call_kwargs = email_client.send_email.call_args
    assert "institution@example.com" in str(call_kwargs)
...
async def test_no_alert_when_org_unit_has_relation(
    ...
    """Create two org units with a relation between them and verify no alert
    is sent for the lønorg unit."""
    org_unit_type = (await graphql_client._testing__get_org_unit_type()).objects[0].uuid
    ...
    await handle_org_unit(context, loen_child.uuid, None, graphql_client, session)
...
@pytest.mark.integration_test
@pytest.mark.envvar({"ALERT_MANAGER_REMOVAL_USE_ORG_UNIT_EMAILS": "false"})
async def test_deduplication_persists_in_database(
    context,
    session: AsyncSession,
    graphql_client: GraphQLClient,
    email_client: MagicMock,
    ...
    """Calling the handler twice for the same org unit should only send one
    email, because the SentAlert is stored in the database."""
    org_unit_type = (await graphql_client._testing__get_org_unit_type()).objects[0].uuid
...
async def test_nonexistent_org_unit_no_email(
    ...
    """When the org unit UUID does not exist, no email is sent."""
    await handle_org_unit(context, uuid4(), None, graphql_client, session)
    email_client.send_email.assert_not_called()
...
async def test_org_unit_outside_loenorg_no_email(
    ...
    # Create the lønorg root so the setting is valid
    await graphql_client._testing__create_org_unit_root(
        name="Lønorganisation",
        root_uuid=root_loen_org,
        org_unit_type=org_unit_type,
    ...
    # Create a separate root and child outside the lønorg
    other_root = uuid4()
    await graphql_client._testing__create_org_unit_root(
        name="Administrationsorganisation",
...
async def test_root_loenorg_uses_own_email(
    ...
    await handle_org_unit(context, root_loen_org, None, graphql_client, session)

    email_client.send_email.assert_called_once()
    call_kwargs = email_client.send_email.call_args
    assert "loenorg-root@example.com" in str(call_kwargs)
