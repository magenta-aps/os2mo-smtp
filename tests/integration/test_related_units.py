...
async def test_no_alert_when_relation_exists(
    ...
    # The mutation's returned uuid is not the related_unit object — query for it
    related = (
...
@pytest.mark.integration_test
@pytest.mark.envvar({"ALERT_MANAGER_REMOVAL_USE_ORG_UNIT_EMAILS": "false"})
async def test_handle_related_units_deduplicates(
    context,
    session: AsyncSession,
    graphql_client: GraphQLClient,
    email_client: MagicMock,
    ...
    """Calling handle_related_units twice after a relation is deleted should
    only send one email thanks to DB-backed deduplication."""
    org_unit_type = (await graphql_client._testing__get_org_unit_type()).objects[0].uuid
